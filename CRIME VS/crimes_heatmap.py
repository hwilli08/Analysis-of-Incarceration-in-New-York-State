#This script analyzes the relationship between the top 10 crimes and both the top 25 housing facilities and race/ethnicity of incarcerated individuals in 2023, visualizing the data using heatmaps.
...
#Geting the top 25 facilities with the most incarcerated individuals.
top_facilities = incarcerated_data_2023.groupby('Housing Facility').size().nlargest(25).index

#Filtering the dataset to include only the top 25 facilities.
...
#Grouping by facility and crime.
facility_crime_counts = incarcerated_data_2023.groupby(['Housing Facility', 'Most Serious Crime']).size().unstack(fill_value=0)

#Plotting a heatmap for relationship between facility and crime.
plt.figure(figsize=(12, 8))
sns.heatmap(facility_crime_counts, cmap='YlOrBr', annot=True, fmt='d')
plt.title('Relationship between Top 10 Most Serious Crimes Committed and Top 25 NYS Housing Facilities (2023)')
plt.xlabel('Most Serious Crime')
plt.ylabel('Housing Facility')
